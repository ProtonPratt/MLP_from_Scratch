...
class MLP:
    ...
    def __init__(self, input_size, hidden_layers, output_size, epochs = 100, learning_rate = 0.01, early_stopping = False,
                 batch_size = 32, activation_function = 'relu', loss_function = 'mse', optimizer = 'sgd', beta = 0.0, wandb_finish = True,
                 Random_state = None, weight_init = 'he', model_type = 'class_ML', wandb_vaar = True, run_start = "hyperparam_tuning"):
...
# run the model for various hyperparameters
...
def run_model(params, X_train, X_test, X_val, y_train, y_test, y_val):
    ...
    activation_function, loss_function, batch_size, learning_rate, beta, hidden_layers = params
    mlp = MLP(11, hidden_layers, 6, epochs = 1000, learning_rate = learning_rate, batch_size = batch_size, activation_function = activation_function, early_stopping=True,
          loss_function = 'cross_entropy', optimizer = 'mini_batch', beta = beta, Random_state=None, model_type='class_MC', wandb_vaar=False, run_start="hyperparam_final-2", wandb_finish=False)
    ...
    print(f"Activation: {activation_function}, Loss: {loss_function}, Batch: {batch_size}, Learning rate: {learning_rate}, Beta: {beta}, Hidden layers: {hidden_layers}, Time: {run_time}")
    
    # on validation data
    accuracy = np.mean(y_pred == y_val)
    f1_score = mlp.f1_score(y_val, y_pred)
    precision = mlp.precision(y_val, y_pred)
    recall = mlp.recall(y_val, y_pred)
    
    print("Validation data")
    print(f"Accuracy: {accuracy}, F1 Score: {f1_score}, Precision: {precision}, Recall: {recall}")
    
    # print(f"gradient check: {mlp.check_gradients(X_val, y_val)}")
    
    # wandb.log({"data": "Validation", "accuracy": accuracy, "f1_score": f1_score, "precision": precision, "recall": recall})
    ...
    # wandb.log({"data": "Test", "accuracy": accuracy, "f1_score": f1_score, "precision": precision, "recall": recall})
    
    # wandb.finish()
    
    return f"Done: Activation={activation_function}, Loss={loss_function}, Batch={batch_size}, Learning_rate={learning_rate}, Beta={beta}, Hidden_layers={hidden_layers}"

# create the parameter grid
params = list(itertools.product(activation_functions, ['cross_entropy'], batch_sizes, learning_rates, beta, hidden_layers))

for i in range(len(params)):
    run_model(params[i], X_train, X_test, X_val, y_train, y_test, y_val)